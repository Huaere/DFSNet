...
from Utils.dataloader_bag import TrainDataset_Fold_twoscale_innov,ValDataset_Fold,ValDataset_Fold_twoscale_innov,TrainDataset_Fold_4010_innov,ValDataset_Fold_4010_innov,TrainDataset_Fold_twoscale_sliderandom_innov
# from Utils.model import  my_resnet_10x,keyBlock
from Utils.model_multiLayer import  keyBlock_new

from Utils.train_utils import train_model,  make_folder_two_scale_slide,make_two_scale_test_folder,train_two_scale_model
...
if not os.path.exists('Z:/admin/LXH/lxh_train_code/mutation/Checkpoint_targeted_surgery_two_scale_norm_balance_copy'):
    os.makedirs('Z:/admin/LXH/lxh_train_code/mutation/Checkpoint_targeted_surgery_two_scale_norm_balance_copy')
...
if len(os.listdir('Z:/admin/LXH/lxh_train_code/mutation/Val_list_surgery_two_scale_norm_balance')) < 6:
    ...
    for i_fold in range (1,3):
        ...
        train_file = 'Z:/admin/LXH/lxh_train_code/mutation/Val_list_surgery_two_scale_norm_balance/train_folder'+str(i_fold)+'.json'
        with open(train_file, 'r') as f:
            train_folder_ = json.load(f)

        val_file = 'Z:/admin/LXH/lxh_train_code/mutation/Val_list_surgery_two_scale_norm_balance/val_folder'+str(i_fold)+'.json'
        with open(val_file, 'r') as f:
            val_folder_ = json.load(f)
    
        print('k_fold:',i_fold)
        image_datasets = {'train': TrainDataset_Fold_twoscale_sliderandom_innov(train_folder_, transform_original['train'], transform_down['train'],bag_size,ratio=1), 'val': ValDataset_Fold_twoscale_innov(val_folder_,transform_original['train'], transform_down['train'],bag_size,ratio=1)}
       ...
       ## 删除不需要的权重
        ...
        model_ft = train_two_scale_model(model_ft, criterion, optimizer_ft, dataloaders, dataset_sizes, epoch, writer,scheduler)
        torch.save(model_ft.state_dict(), os.path.join('Z:/admin/LXH/lxh_train_code/mutation/Checkpoint_targeted_surgery_two_scale_norm_balance_copy', 'resnet34_two_scale_10_10down_drop_nearest_fold_' + str(i_fold) + '.pt'))
